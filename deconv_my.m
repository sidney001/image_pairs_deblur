...
niter = 50;
alpha = 0.20;    % alpha controls the influence of the gain map
winr  = 5;
sig_d = 1.6;
sig_r = 0.08;
...
%             NdK(:,:,i) = conv2(Nd(:,:,i), K, 'same');
        ...
        dB = B - NdK;
        dI = residual_Lucy(dB, K, niter);
%         dI = deconvlucy(dB, K, niter, 0) - 1; % matlab builtin RL
        I = Nd + dI;
        figure;imshow(I,[]);title('residual RL deconvoltion');
        figure;imshow(dI+0.5,[ ]);title('delta I--- residual RL');
    case 'gcRL' % gain-controlled RL
        ...
        dB = B - NdK; % add 1 as offset
        dI = gain_residual_Lucy(Nd, dB, K, niter, alpha);
        I = Nd + dI;
        figure;imshow(dI+0.5,[ ]);title('delta I--- gain-controlled RL');
